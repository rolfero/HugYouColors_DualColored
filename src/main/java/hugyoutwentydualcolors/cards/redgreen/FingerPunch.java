...
public class FingerPunch extends DualCard {
    private final static CardInfo cardInfo = new CardInfo(
            "FingerPunch", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public float calculateModifiedCardDamage(AbstractPlayer player, AbstractMonster mo, float tmp) {
        if (player.hasPower(DexterityPower.POWER_ID))
            return tmp + player.getPower(DexterityPower.POWER_ID).amount;
        return tmp;
    }
