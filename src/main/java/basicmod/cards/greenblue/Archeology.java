...
import basicmod.powers.ArcheologyPower;
...
public class Archeology extends DualCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Archeology", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public Archeology() {
        ...
        setCostUpgrade(1);
        setMagic(MAGIC);
        cardsToPreview = new Claw();
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new ArcheologyPower(p, magicNumber), magicNumber));
    }
}