package hugyoucolors.cards.greenblue;

import hugyoucolors.HugYouColors;
import hugyoucolors.actions.DamageLowestHealthAction;
...
public class TargetedBlast extends DualCard {
    private final static CardInfo cardInfo = new CardInfo(
            "TargetedBlast", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new DamageLowestHealthAction(new DamageInfo(p, damage, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.BLUNT_LIGHT));
    }
}