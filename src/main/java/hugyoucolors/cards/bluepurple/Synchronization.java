package hugyoucolors.cards.bluepurple;

import hugyoucolors.HugYouColors;
import hugyoucolors.cards.DualCard;
import hugyoucolors.powers.SynchronizationPower;
...
public class Synchronization extends DualCard {
    private final static CardInfo cardInfo = new CardInfo(
            "Synchronization", //Card ID. Will be prefixed with mod id, so the final ID will be "modID:MyCard" with whatever your mod's ID is.
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new SynchronizationPower(p, 1), 1));
    }
}